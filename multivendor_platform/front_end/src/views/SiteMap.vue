<template>
  <v-container class="sitemap-page py-8">
    <!-- Page Header -->
    <v-row class="mb-8">
      <v-col cols="12">
        ...
        <v-card elevation="2" class="h-100">
          ...
          <v-card-text>
            <v-list density="compact" nav>
              ...
              <v-list-item v-if="isAuthenticated && isSeller" :to="{ path: '/products/new' }" prepend-icon="mdi-plus-circle" link>
                <v-list-item-title>افزودن محصول جدید</v-list-item-title>
              </v-list-item>
              <v-list-item v-if="isAuthenticated && isSeller" :to="{ path: '/my-products' }" prepend-icon="mdi-package-variant-closed" link>
                <v-list-item-title>محصولات من</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-card-text>
        </v-card>
          ...
          <v-card-text>
            <v-list density="compact" nav>
              ...
              <v-list-item 
                ...
                link
              >
                <v-list-item-title>{{ supplier.store_name }}</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-card-text>
        </v-card>
        ...
        <v-card elevation="2" class="h-100">
          ...
          <v-card-text>
            <v-list density="compact">
              ...
              <v-list-item v-if="isAuthenticated" :to="{ path: '/blog/dashboard' }" prepend-icon="mdi-view-dashboard" link>
                <v-list-item-title>داشبورد وبلاگ</v-list-item-title>
              </v-list-item>
              <v-list-item v-if="isAuthenticated" :to="{ path: '/blog/new' }" prepend-icon="mdi-pencil-plus" link>
        ...
        <v-card elevation="2" class="h-100">
          ...
          <v-card-text>
            ...
            <v-list density="compact" v-else nav>
              ...
              <v-list-item v-if="isAdmin" @click="goToAdminPanel" prepend-icon="mdi-shield-account" link>
...
const sitemapXmlUrl = computed(() => {
  return `${config.apiUrl.replace('/api', '')}/sitemap.xml`
})
...
const fetchSuppliers = async () => {
  try {
    const response = await api.get('/users/suppliers/')
...
const fetchStats = async () => {
  try {
    ...
    const productsResponse = await api.get('/products/', { params: { page_size: 1 } })
    stats.value.totalProducts = productsResponse.data.count || 0

    // Fetch blog posts count
    const blogResponse = await api.get('/blog/posts/', { params: { page_size: 1 } })
    stats.value.totalBlogPosts = blogResponse.data.count || 0
  } catch (error) {
