...
def create_product_from_scraped_data(scraped_data, vendor, supplier=None):
    """
    Create a Product instance from scraped data with unique slug
    """
    from .models import Product, ProductImage
    import random
    
    try:
        # Generate unique slug with 5-digit random number
        # This prevents conflicts when scraping similar product names
        ...
        while Product.objects.filter(slug=unique_slug).exists():
            ...
            if collision_count > 10:
                # Very unlikely, but prevent infinite loop
                import time
        ...
        product = Product(
            ...
            is_active=False,  # Set as draft initially
        )
        product.save()
