...
if [ "$response" = "403" ]; then
    ...
    echo ""
    echo "2. Check VPS Docker Logs:"
    echo "   SSH to your VPS and run:"
    echo "   docker-compose logs backend"
    echo "   docker-compose ps"
    echo ""
    echo "3. If deployment succeeded but still 403:"
    echo "   - SSH to VPS: ssh user@your-vps-ip"
    echo "   - Navigate to project: cd /path/to/project"
    echo "   - Restart services: docker-compose restart backend"
    echo "   - Or rebuild: docker-compose up -d --build"
    echo ""
else
